...
async def generate_and_index_adapters(
    ...
    Updates the 'adapter_jobs' status at each stage so the progress can be tracked.
    """
    conn_str = str(connection_id)

    # Initialize/Reset Job Status in DB
    await db["adapter_jobs"].update_one(
        {"connection_id": conn_str},
        {
            "$set": {
                "status": "running",
                "progress": "Initializing connection and loading schema...",
    ...
    try:
        ...
        await db["adapter_jobs"].update_one(
            ...
            {
                "$set": {
                    "progress": f"Mapping {len(tables)} tables/collections using DeepSeek AI...",
                    "updated_at": datetime.now(UTC)
                }
            }
        )
        ...
        async def process_chunk(idx, chunk):
            ...
            user_prompt = f"""Analyze the following database tables/collections and map them to standard business entities (e.g. customer, supplier, invoice, inventory_item, warehouse, sales_order, purchase_order, general_ledger, WIP, etc.).
...
4. Output ONLY valid raw JSON matching the schema. No markdown formatting or extra text."""
        ...
        await db["adapter_jobs"].update_one(
            ...
            {
                "$set": {
                    "progress": "Generating ontology and mapping configurations...",
                    "updated_at": datetime.now(UTC)
                }
            }
        )
        ...
        await db["adapter_jobs"].update_one(
            ...
            {
                "$set": {
                    "progress": "Preparing vector index representations...",
                    "updated_at": datetime.now(UTC)
                }
            }
        )
        ...
        await db["adapter_jobs"].update_one(
            ...
            {
                "$set": {
                    "progress": f"Uploading {len(vectors_to_upsert)} vector embeddings to Pinecone...",
                    "updated_at": datetime.now(UTC)
                }
            }
        )
        ...
        await db["adapter_jobs"].update_one(
            {"connection_id": conn_str},
            {
                "$set": {
                    "status": "success",
                    "progress": "Semantic adapters and Pinecone index successfully generated!",
                    "updated_at": datetime.now(UTC),
                    "result": res_data
                }
            }
        ...
        log.error("adapter_generation_failed", extra={"conn_id": conn_str, "error": str(e)}, exc_info=True)
