...
async def signup(data: SignupRequest, db: AsyncIOMotorDatabase = Depends(get_db)) -> AuthResponse:
...
async def login(data: LoginRequest, db: AsyncIOMotorDatabase = Depends(get_db)) -> AuthResponse:
    return await auth_service.login(db, data)


@router.post("/send-otp", status_code=status.HTTP_200_OK)
async def send_otp(
    data: SendOtpRequest, db: AsyncIOMotorDatabase = Depends(get_db)
) -> dict:
...
async def forgot_password(
    data: ForgotPasswordRequest, db: AsyncIOMotorDatabase = Depends(get_db)
) -> dict:
...
async def refresh(data: RefreshRequest, db: AsyncIOMotorDatabase = Depends(get_db)) -> TokenPair:
...
async def accept_invite(
    data: AcceptInviteRequest, db: AsyncIOMotorDatabase = Depends(get_db)
) -> AuthResponse:
    return await invitation_service.accept(db, data)


@router.get("/invite", response_model=InvitePreview)
async def invite_preview(token: str, db: AsyncIOMotorDatabase = Depends(get_db)) -> InvitePreview:
