...
ENTRYPOINT ["tini", "--"]
CMD ["sh", "-c", "if [ \"$(echo $RUN_MIGRATIONS | tr '[:upper:]' '[:lower:]')\" = \"true\" ]; then alembic -c app/migrations/alembic.ini upgrade head; fi && exec uvicorn app.main:app --host 0.0.0.0 --port 8000 --proxy-headers --forwarded-allow-ips='*'"]
